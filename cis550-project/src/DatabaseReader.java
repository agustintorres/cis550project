...
public class DatabaseReader {
	...
	public ArrayList<Comment> getCommentsByUser(String uid){
		...
		try {
			...
			while(rs.next()){
				...
				String mytime = rs.getString("commenttime");
				String mytitle = rs.getString("title");
				String mylink = rs.getString("url");

				//System.out.println(mytext + myauthor + mytime + mytitle + mylink);

				tempComment.setText(mytext);
				tempComment.setAuthor(myauthor);
				tempComment.setCommenttime(mytime);
				tempComment.setStorytitle(mytitle);
				tempComment.setStorylink(mylink);
	...
	public ArrayList<String> getTopUsers(){
		...
		ArrayList<Double> tempScores2 = new ArrayList<Double>();
		ArrayList<String> names = new ArrayList<String>();

		try {
			Connection conn = getConnection();

			PreparedStatement pstmt = conn.prepareStatement("SELECT name FROM STORIES");
			...
			pstmt = conn.prepareStatement("SELECT storyid FROM STORIES WHERE name = ?");
			...
			for(int x = 0; x < names.size(); x++){
				...
				while(rs.next()){
					storyCount = storyCount + 1.0;
					voteCount += getVotesByStory(rs.getInt(1));
				}

				voteCount = voteCount + 0.0;
				Double ratio = voteCount/storyCount + 0.0;

				tempScores.add(ratio);
				tempNames.add(names.get(x));
			}

			for(int x = 0; x < tempScores.size(); x++){
				tempScores2.add(tempScores.get(x));
			}

			Collections.sort(tempScores);

			for(int x = tempScores.size() -1; x >= 0; x--){
				for(int y = 0; y < tempScores2.size(); y++){
					if(tempScores2.get(y) == tempScores.get(x)){
						results.add(tempNames.get(y));
						results.add(tempScores.get(x).toString());
					}
				}
			}
